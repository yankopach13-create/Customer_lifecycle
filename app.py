"""
Жизненный цикл клиента в продукте.
Streamlit-приложение для загрузки отчётов из Qlik по шаблону.
"""

import streamlit as st

# --- Конфигурация страницы (для Streamlit Cloud) ---
st.set_page_config(
    page_title="Жизненный цикл клиента",
    page_icon="📊",
    layout="wide",
)

# --- Заголовок ---
st.title("Жизненный цикл клиента в продукте")

# --- Две колонки: инструкция слева, шаблон и загрузчик справа ---
col_instruction, col_template = st.columns([1, 1])

with col_instruction:
    st.subheader("Инструкция")
    st.markdown("""
    1. Зайдите в **Qlik**, раздел «Анализ чеков».
    2. Отберите **необходимую категорию** и **уровни товара**.
    3. Отберите **анализируемый период**.
    4. Зайдите на лист **«Конструктор»** и выведите отчёт по шаблону справа.
    5. Настройте фильтрами построение динамики когорт: **Год–Месяц** или **Год–Неделя**.
    6. Скачайте документ в Qlik и загрузите в ячейку справа.
    """)

with col_template:
    st.subheader("📋 Шаблон загрузки данных из Qlik")
    try:
        st.image("qlik_template_categories.png", use_container_width=True)
    except FileNotFoundError:
        st.warning("Изображение шаблона не найдено. Убедитесь, что файл `qlik_template_categories.png` в корне проекта.")
    st.markdown("---")
    st.caption("Загрузите выгруженный из Qlik документ (Excel):")
    uploaded_file = st.file_uploader(
        "Выберите файл",
        type=["xlsx", "xls"],
        key="qlik_upload",
        label_visibility="collapsed",
    )
    if uploaded_file:
        st.success(f"Загружен файл: **{uploaded_file.name}**")
        # Здесь можно добавить обработку файла (чтение, превью и т.д.)
